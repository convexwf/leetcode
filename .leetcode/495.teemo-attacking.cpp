...
// 38/38 cases passed
// Runtime: 36 ms, faster than 89.98% of C++ online submissions.
// Memory Usage: 25.9 MB, less than 44.99% of C++ online submissions.
class Solution {
public:
    int findPoisonedDuration(vector<int>& timeSeries, int duration) {
        ...
        for (int ts : timeSeries) {
            int cur = ts + duration - 1;
            if (cur > last) {
                res += (cur - max(ts-1, last));
                last = cur;
            }
        }
        return res;
