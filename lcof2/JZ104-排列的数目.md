...
1. 使用动态规划，`dp[i]` 表示凑成目标值 `i` 的排列数。
2. 初始化 `dp[0] = 1`，表示凑成 0 有一种方式（一个数都不选）。
3. 外层循环遍历所有可能的目标值，内层循环遍历 `nums` 数组。
4. 状态转移方程：`dp[i] += dp[i - num]`，表示如果选择 `num`，剩余值的排列数。
5. 注意这里考虑顺序，所以外层遍历目标值，内层遍历 `nums`。
