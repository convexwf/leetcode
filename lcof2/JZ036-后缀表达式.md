...
1. 使用栈来处理逆波兰表达式。
2. 遍历tokens数组，如果是数字则入栈。
3. 如果是运算符，弹出栈顶两个元素进行计算（注意顺序，先弹出的是右操作数）。
4. 将计算结果压入栈中。
5. 最后栈中剩余的唯一元素就是表达式的结果。

时间复杂度：$O(n)$，其中 $n$ 是tokens数组的长度，需要遍历一次。空间复杂度：$O(n)$，栈最多存储 $n/2$ 个元素。
