...
1. 环形数组的特点是第一个房子和最后一个房子不能同时偷，需要分两种情况讨论。
2. 情况1：偷第一个房子，则不能偷最后一个，范围为 `[0, n-2]`。
3. 情况2：不偷第一个房子，则可以考虑最后一个，范围为 `[1, n-1]`。
4. 对每种情况使用线性房屋偷盗的动态规划方法，`dp[i] = max(dp[i-1], dp[i-2] + nums[i])`。
5. 返回两种情况的最大值，时间复杂度 $O(n)$。
