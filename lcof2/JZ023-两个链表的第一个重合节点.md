# 剑指 Offer II JZ023-两个链表的第一个重合节点
...
1. 使用双指针法，两个指针分别从两个链表的头部开始遍历。
2. 当指针到达链表尾部时，跳转到另一个链表的头部继续遍历。
3. 如果两个链表相交，两个指针最终会在交点相遇。
4. 如果不相交，两个指针最终都会到达null。
5. 这个方法的核心是消除两个链表的长度差。

时间复杂度：$O(m + n)$，其中 $m$ 和 $n$ 是两个链表的长度。空间复杂度：$O(1)$，只使用两个指针。
...
public:
    ListNode *getIntersectionNode(ListNode *headA, ListNode *headB) {
        ...
        // When pointers reach end, jump to other list's head
        while (pA != pB) {
