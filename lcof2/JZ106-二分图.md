...
1. 使用 BFS 广度优先搜索进行图染色，尝试将图分为两部分。
2. 使用数组 `colors` 记录每个节点的颜色，-1 表示未染色，0 和 1 表示两种颜色。
3. 对于每个未染色的节点，从该节点开始 BFS，将其染为颜色 0。
4. BFS 过程中，相邻节点应染成不同颜色，如果发现相邻节点颜色相同则不是二分图。
5. 如果所有节点都能成功染色且没有冲突，返回 `true`。
...
private:
    bool bfs(vector<vector<int>>& graph, int start, vector<int>& colors) {
        queue<int> q;
        q.push(start);
        colors[start] = 0;
