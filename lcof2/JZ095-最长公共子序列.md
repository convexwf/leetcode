...
1. 使用动态规划，`dp[i][j]` 表示 `text1` 的前 `i` 个字符和 `text2` 的前 `j` 个字符的最长公共子序列长度。
2. 初始化 `dp[0][j] = 0` 和 `dp[i][0] = 0`，表示空字符串的 LCS 为 0。
3. 如果 `text1[i-1] == text2[j-1]`，则 `dp[i][j] = dp[i-1][j-1] + 1`。
4. 否则 `dp[i][j] = max(dp[i-1][j], dp[i][j-1])`，选择不包含当前字符的最大值。
5. 最终返回 `dp[len1][len2]`。
