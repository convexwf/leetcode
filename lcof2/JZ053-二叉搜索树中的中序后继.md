...
1. 利用二叉搜索树的性质，中序后继是比 `p.val` 大的最小节点。
2. 从根节点开始查找，如果当前节点值大于 `p.val`，说明后继可能在左子树，记录当前节点并继续向左查找。
3. 如果当前节点值小于等于 `p.val`，说明后继一定在右子树，向右查找。
4. 最终返回记录的后继节点。

时间复杂度：$O(h)$，其中 $h$ 是树的高度，最多访问树的高度次。空间复杂度：$O(1)$，只使用常数额外空间。
    ...
    TreeNode* inorderSuccessor(TreeNode* root, TreeNode* p) {
        ...
        while (curr != nullptr) {
            if (curr->val > p->val) {
                ...
                // Need to find larger value, go right
                curr = curr->right;
            }
        }
