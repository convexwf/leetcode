# 剑指 Offer II JZ003-前 n 个数字二进制中 1 的个数
...
1. 使用动态规划，`result[i]` 表示数字 `i` 的二进制表示中 1 的个数。
2. 利用状态转移方程：`result[i] = result[i >> 1] + (i & 1)`。
3. `i >> 1` 表示将 `i` 右移一位（相当于除以 2），`i & 1` 表示 `i` 的最低位。
4. 因为 `i` 的 1 的个数等于 `i >> 1` 的 1 的个数加上 `i` 的最低位。
