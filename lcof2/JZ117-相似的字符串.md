...
1. 使用并查集将相似的字符串归并到同一组中，初始化每个字符串为独立的组。
2. 对所有字符串对进行两两比较，使用 `isSimilar` 函数判断是否相似。
3. `isSimilar` 函数遍历字符串，统计不同位置的字符个数，只有差异为 0（完全相同）或 2（可通过一次交换变相同）时才相似。
4. 如果两个字符串相似，则通过 `unionSet` 将它们合并到同一组。
5. 遍历所有字符串，通过 `find` 操作找到各自的根节点，使用 `unordered_set` 统计不同根节点的数量即为分组数。
