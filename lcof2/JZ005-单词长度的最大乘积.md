...
1. 使用位运算将每个单词的字符集合压缩成一个整数 `mask`，第 `i` 位为 1 表示包含字符 `'a' + i`。
2. 遍历每个单词，对其中每个字符 `c`，将 `mask` 的第 `c - 'a'` 位置为 1。
3. 对所有单词对进行两两比较，使用按位与运算 `masks[i] & masks[j]` 判断是否有公共字符。
4. 如果没有公共字符（结果为 0），计算两个单词长度的乘积，更新最大值。
5. 时间复杂度 $O(n^2 + L)$，其中 $L$ 为所有单词的总长度。
