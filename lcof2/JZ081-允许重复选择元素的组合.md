...
1. 使用回溯算法，每个元素可以重复使用多次。
2. 为了避免重复组合，使用 `start` 参数确保每次递归从当前位置开始。
3. 当目标值为 0 时，将当前路径加入结果集。
4. 剪枝优化：当当前元素大于目标值时，直接跳出循环。
5. 因为元素可重复使用，下次递归仍从 `i` 开始。
