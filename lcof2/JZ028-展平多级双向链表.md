...
1. 使用深度优先搜索（DFS）或迭代方法处理多级链表。
2. 遍历链表，当遇到有child指针的节点时：
   - 保存next节点。
   - 递归展平child链表，将其插入到当前节点之后。
   - 连接展平后的child链表的尾部和之前保存的next节点。
3. 将child指针置为null。
4. 继续处理后续节点。

时间复杂度：$O(n)$，其中 $n$ 是所有节点的总数，每个节点访问一次。空间复杂度：$O(n)$，递归调用栈的深度。
