...
1. 先处理溢出情况：当 `a` 为 `INT_MIN` 且 `b` 为 -1 时，结果会溢出，返回 `INT_MAX`。
2. 判断结果的符号，用异或 `^` 判断两个数符号是否不同。
3. 将两个数都转为负数进行计算（避免正数转换时的溢出）。
4. 使用倍增的思想：每次将除数倍增（左移），直到不能再倍增，然后从被除数中减去。
5. 累加倍数到结果中，最后根据符号返回结果。

```cpp
class Solution {
public:
    int divide(int a, int b) {
        // Check for overflow: INT_MIN=-2^31, INT_MAX=2^31-1
        if (a == INT_MIN && b == -1) {
            return INT_MAX;
        }
