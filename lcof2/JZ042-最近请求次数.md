...
1. 使用队列存储所有请求的时间戳，保持队列有序（因为请求时间严格递增）。
2. 每次调用 `ping(t)` 时，将新的时间 `t` 加入队列。
3. 从队首开始移除所有不在 `[t-3000, t]` 范围内的旧请求。
4. 返回队列中剩余元素的数量，即为3000毫秒内的请求数。

时间复杂度：均摊 $O(1)$，每个元素最多入队一次出队一次。空间复杂度：$O(n)$，其中 $n$ 是3000毫秒内的最大请求数。
