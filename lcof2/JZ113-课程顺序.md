...
1. 使用 BFS 进行拓扑排序，构建邻接表和入度数组。
2. 遍历 `prerequisites`，对于每对 `[a, b]`，表示 `b -> a` 的边，即先修 `b` 才能修 `a`。
3. 将所有入度为 0 的课程加入队列，作为起始课程。
4. BFS 过程中，每次取出一个课程，将其后继课程的入度减 1，入度为 0 时入队。
5. 如果所有课程都被访问，返回结果；否则说明有环，返回空数组。
