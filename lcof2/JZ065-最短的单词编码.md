...
1. 使用后缀树（反向 Trie），将所有单词的反向插入 Trie 中。
2. 如果一个单词是另一个单词的后缀，则它们在 Trie 中共享路径。
3. 只有叶子节点对应的单词需要单独编码（不是其他单词的后缀）。
4. 统计所有叶子节点的深度加 1（`#` 字符）的总和。
5. 时间复杂度 $O(n \cdot k)$，空间复杂度 $O(n \cdot k)$，其中 `k` 是单词平均长度。
...
public:
    int minimumLengthEncoding(vector<string>& words) {
        ...
        // Insert all words in reverse order
        ...
        for (auto& [node, idx] : nodes) {
            // If it's a leaf node (not a suffix of other words)
            if (node->children.empty()) {
                length += words[idx].size() + 1; // +1 for '#'
            }
        }
