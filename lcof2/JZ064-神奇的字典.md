...
1. 使用哈希表按长度分组存储字典中的单词。
2. `search` 时，首先查找与查询单词长度相同的所有单词。
3. 对于每个相同长度的单词，检查是否恰好有一个字符不同。
4. 如果找到恰好有一个字符不同的单词，返回 `true`。
5. 时间复杂度：`buildDict` $O(n \cdot k)$，`search` $O(m \cdot k)$，其中 `k` 是单词长度。
...
public:
    ...
    bool search(string searchWord) {
        ...
        if (!dict.count(len)) return false;
