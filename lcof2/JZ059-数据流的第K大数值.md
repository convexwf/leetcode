...
1. 使用小根堆维护数据流中的前 `k` 大元素。
2. 堆的大小始终保持为 `k`，堆顶元素就是第 `k` 大的元素。
3. `add` 操作：将新元素加入堆，如果堆大小超过 `k`，弹出堆顶元素。
4. 返回堆顶元素，即为当前第 `k` 大的元素。
5. 时间复杂度：构造 $O(n\log k)$，`add` $O(\log k)$。
