...
1. 将问题建模为带权图，变量为节点，除法关系为边和权重。
2. 使用哈希表建立邻接表，对于 `a / b = value`，建立 `a -> b` 权重为 `value` 和 `b -> a` 权重为 `1/value` 的边。
3. 对于每个查询，使用 DFS 从起点搜索到终点，累乘路径上的权重。
4. 如果起点或终点不存在，或者找不到路径，返回 -1.0。
5. 使用访问标记数组避免重复访问。
    ...
    vector<double> calcEquation(vector<vector<string>>& equations, vector<double>& values, vector<vector<string>>& queries) {
        ...
        for (const auto& query : queries) {
            ...
            if (graph.find(start) == graph.end() || graph.find(end) == graph.end()) {
                ...
                double res = dfs(graph, start, end, visited);
