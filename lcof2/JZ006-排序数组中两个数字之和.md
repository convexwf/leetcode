...
1. 利用数组已经按升序排列的特性，使用双指针方法。
2. 初始化 `left` 指针指向数组开头，`right` 指针指向数组末尾。
3. 计算两个指针指向元素的和 `sum`，如果等于 `target`，返回两个下标。
4. 如果 `sum` 小于 `target`，说明需要更大的数，`left` 右移。
5. 如果 `sum` 大于 `target`，说明需要更小的数，`right` 左移。
