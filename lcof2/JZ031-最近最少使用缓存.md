...
1. 使用双向链表+哈希表实现LRU缓存。
2. 双向链表按访问时间顺序存储节点，最近访问的在头部，最久未使用的在尾部。
3. 哈希表存储key到链表节点的映射，实现 $O(1)$ 查找。
4. `get`操作：如果key存在，将节点移到链表头部并返回值；否则返回-1。
5. `put`操作：如果key存在，更新值并移到头部；否则插入新节点，如果超出容量则删除尾部节点。

时间复杂度：`get`和`put`操作都是 $O(1)$。空间复杂度：$O(capacity)$，存储最多capacity个节点。
