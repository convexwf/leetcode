...
1. 使用并查集数据结构来检测环路，初始化时每个节点的父节点是自己。
2. 遍历每条边，对边的两个顶点执行 `find` 操作找到它们的根节点。
3. 如果两个顶点的根节点相同，说明它们已经在同一个连通分量中，再添加这条边会形成环，返回这条边。
4. 否则执行 `union` 操作将两个顶点合并到同一个连通分量中。
5. `find` 操作使用路径压缩优化，将查找路径上的所有节点直接指向根节点，提升后续查找效率。
