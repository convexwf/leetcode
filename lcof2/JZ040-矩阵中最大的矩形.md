...
<p><meta charset="UTF-8" />注意：本题与主站 85 题相同（输入参数格式不同）：<a href="https://leetcode.cn/problems/maximal-rectangle/">https://leetcode.cn/problems/maximal-rectangle/</a></p>

<!-- description:end -->

## 解题思路

1. 将问题转化为多个"直方图最大矩形面积"问题（JZ039）。
2. 对于每一行，计算以该行为底的直方图的高度数组（连续1的个数）。
3. 对每个直方图使用单调栈求最大矩形面积。
4. 遍历所有行，取所有直方图的最大面积。

时间复杂度：$O(m \times n)$，其中 $m$ 和 $n$ 分别是矩阵的行数和列数。空间复杂度：$O(n)$，需要存储高度数组和单调栈。
