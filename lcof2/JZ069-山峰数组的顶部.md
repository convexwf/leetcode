...
1. 使用二分查找，在山峰数组中查找峰顶位置。
2. 比较 `arr[mid]` 与 `arr[mid+1]` 的大小。
3. 如果 `arr[mid] < arr[mid+1]`，说明处于上升阶段，峰顶在右侧。
4. 否则说明处于下降阶段或峰顶，峰顶在左侧或当前位置。
    ...
    int peakIndexInMountainArray(vector<int>& arr) {
        ...
        while (left < right) {
            ...
            if (arr[mid] < arr[mid + 1]) {
                // Ascending, peak is on the right
                left = mid + 1;
            } else {
                // Descending or peak, peak is on the left or at mid
                right = mid;
            }
        }
