...
Let me restate the problem first. We need to find two indices in an array such that the sum of the elements at those indices equals a given target. Each input has exactly one solution, and we cannot use the same element twice.

The key idea is to use a hash map to store the numbers we have seen so far along with their indices. As we iterate through the array, for each number, we calculate its complement (target - current number) and check if this complement exists in the hash map. If it does, we have found our two indices.

This approach takes linear time O(n) and linear space O(n).

For example, given nums = [2,7,11,15] and target = 9, we start with an empty hash map. We see 2 first, store it in the map. Next, we see 7, calculate its complement as 2 (9 - 7), which exists in the map. Thus, we return the indices [0, 1].
