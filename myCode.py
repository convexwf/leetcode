...
def extract_doc():
    root_dir = ".doc"
    for doc_file in os.listdir(root_dir):
        problem_id = doc_file.split(".")[0]
        if int(problem_id) > 200:
            continue
        doc_path = os.path.join(root_dir, doc_file)
        ...
        for idx, line in enumerate(lines):
            ...
            if line.startswith("**解题思路**"):
                idx2 = idx
            if line.startswith("**关联专题**"):
                idx3 = idx
        desc = "".join(lines[idx1:idx2])[9:].strip()
        solution = "".join(lines[idx2:idx3])[9:].strip()
        print(desc)
        print(solution)
